...
function activate(ctx){const{theme,settings,commands}=ctx;const dark=l('everforest-dark.json'),light=l('everforest-light.json');
if(dark)ctx.subscriptions.push(theme.register(dark));if(light)ctx.subscriptions.push(theme.register(light));
if(dark)r('everforest-dark','vs-dark',dark);if(light)r('everforest-light','vs',light);
settings.set('workbench.colorTheme','everforest-dark');
ctx.subscriptions.push(theme.onDidChange(id=>{const def=id==='everforest-light'?light:dark;if(def){if(d)d.dispose();d=theme.injectCSS('everforest-vars',bCSS(def))}}));
if(dark){d=theme.injectCSS('everforest-vars',bCSS(dark));ctx.subscriptions.push(d)}
if(commands){ctx.subscriptions.push(commands.registerCommand('everforest.activateDark',()=>settings.set('workbench.colorTheme','everforest-dark')));
ctx.subscriptions.push(commands.registerCommand('everforest.activateLight',()=>settings.set('workbench.colorTheme','everforest-light')))}console.log('[Everforest] activated')}
function deactivate(){if(d){d.dispose();d=null}try{const w=require('electron').BrowserWindow.getAllWindows()[0];if(w&&!w.isDestroyed()){if(m.length)w.webContents.send('ext:monaco:unregister',[...m]);w.webContents.send('ext:theme:revert')}}catch{}m.length=0;console.log('[Everforest] deactivated')}
