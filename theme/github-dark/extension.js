...
function activate(ctx) {
  ...
  const dark = loadTheme('github-dark.json');

  if (dark) ctx.subscriptions.push(theme.register(dark));
  if (dark) registerMonacoTheme('github-dark', 'vs-dark', dark);
  ...
  ctx.subscriptions.push(theme.onDidChange(() => {
    if (dark) {
      ...
      cssDisposable = theme.injectCSS('github-dark-vars', buildCSS(dark));
    }
  }));
...
function deactivate() {
  ...
  console.log('[GitHub Dark] deactivated');
}
