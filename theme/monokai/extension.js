...
function activate(ctx) {
  ...
  const dark = loadTheme('monokai-pro.json');
  if (dark) ctx.subscriptions.push(theme.register(dark));
  if (dark) registerMonacoTheme('monokai-pro', 'vs-dark', dark);
  settings.set('workbench.colorTheme', 'monokai-pro');
  ctx.subscriptions.push(theme.onDidChange(() => { if (dark) { if (cssDisposable) cssDisposable.dispose(); cssDisposable = theme.injectCSS('monokai-vars', buildCSS(dark)); } }));
  if (dark) { cssDisposable = theme.injectCSS('monokai-vars', buildCSS(dark)); ctx.subscriptions.push(cssDisposable); }
  if (commands) { ctx.subscriptions.push(commands.registerCommand('monokai.activate', () => settings.set('workbench.colorTheme', 'monokai-pro'))); }
  console.log('[Monokai] activated');
}
function deactivate() {
  ...
  monacoThemeIds.length = 0; console.log('[Monokai] deactivated');
}
