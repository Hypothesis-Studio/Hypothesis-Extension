...
function activate(ctx) {
  ...
  const dark = loadTheme('solarized-dark.json'); const light = loadTheme('solarized-light.json');
  if (dark) ctx.subscriptions.push(theme.register(dark));
  if (light) ctx.subscriptions.push(theme.register(light));
  if (dark) registerMonacoTheme('solarized-dark', 'vs-dark', dark);
  if (light) registerMonacoTheme('solarized-light', 'vs', light);
  settings.set('workbench.colorTheme', 'solarized-dark');
  ctx.subscriptions.push(theme.onDidChange((id) => { const def = id === 'solarized-light' ? light : dark; if (def) { if (cssDisposable) cssDisposable.dispose(); cssDisposable = theme.injectCSS('solarized-vars', buildCSS(def)); } }));
  if (dark) { cssDisposable = theme.injectCSS('solarized-vars', buildCSS(dark)); ctx.subscriptions.push(cssDisposable); }
  if (commands) {
    ctx.subscriptions.push(commands.registerCommand('solarized.activateDark', () => settings.set('workbench.colorTheme', 'solarized-dark')));
    ctx.subscriptions.push(commands.registerCommand('solarized.activateLight', () => settings.set('workbench.colorTheme', 'solarized-light')));
  }
  console.log('[Solarized] activated');
}
function deactivate() {
  ...
  monacoThemeIds.length = 0; console.log('[Solarized] deactivated');
}
