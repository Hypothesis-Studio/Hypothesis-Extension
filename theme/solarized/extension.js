...
function activate(ctx) {
  ...
  const dark = loadTheme('solarized-dark.json');
  const light = loadTheme('solarized-light.json');
  ...
  ctx.subscriptions.push(theme.onDidChange((id) => {
    const def = id === 'solarized-light' ? light : dark;
    if (def) {
      if (cssDisposable) cssDisposable.dispose();
      cssDisposable = theme.injectCSS('solarized-vars', buildCSS(def));
    }
  }));

  if (dark) {
    cssDisposable = theme.injectCSS('solarized-vars', buildCSS(dark));
...
function deactivate() {
  ...
  console.log('[Solarized] deactivated');
}
