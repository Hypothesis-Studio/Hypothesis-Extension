...
function activate(ctx) {
  ...
  const mocha = loadTheme('catppuccin-mocha.json');
  const latte = loadTheme('catppuccin-latte.json');

  if (mocha) ctx.subscriptions.push(theme.register(mocha));
  if (latte) ctx.subscriptions.push(theme.register(latte));

  if (mocha) registerMonacoTheme('catppuccin-mocha', 'vs-dark', mocha);
  if (latte) registerMonacoTheme('catppuccin-latte', 'vs', latte);

  settings.set('workbench.colorTheme', 'catppuccin-mocha');

  ctx.subscriptions.push(theme.onDidChange((id) => {
    const def = id === 'catppuccin-latte' ? latte : mocha;
    if (def) {
      if (cssDisposable) cssDisposable.dispose();
      cssDisposable = theme.injectCSS('catppuccin-vars', buildCSS(def));
    }
  }));

  if (mocha) {
    cssDisposable = theme.injectCSS('catppuccin-vars', buildCSS(mocha));
    ctx.subscriptions.push(cssDisposable);
  }

  console.log('[Catppuccin] activated');
}
...
function deactivate() {
  ...
  console.log('[Catppuccin] deactivated');
}
