...
function activate(ctx) {
  ...
  const dark = loadTheme('palenight.json');
  if (dark) ctx.subscriptions.push(theme.register(dark));
  if (dark) registerMonacoTheme('palenight', 'vs-dark', dark);
  settings.set('workbench.colorTheme', 'palenight');
  ctx.subscriptions.push(theme.onDidChange(() => { if (dark) { if (cssDisposable) cssDisposable.dispose(); cssDisposable = theme.injectCSS('palenight-vars', buildCSS(dark)); } }));
  if (dark) { cssDisposable = theme.injectCSS('palenight-vars', buildCSS(dark)); ctx.subscriptions.push(cssDisposable); }
  if (commands) { ctx.subscriptions.push(commands.registerCommand('palenight.activate', () => settings.set('workbench.colorTheme', 'palenight'))); }
  console.log('[Palenight] activated');
}
function deactivate() {
  ...
  monacoThemeIds.length = 0; console.log('[Palenight] deactivated');
}
