...
function activate(ctx) {
  ...
  const dark = loadTheme('rose-pine.json');

  if (dark) ctx.subscriptions.push(theme.register(dark));
  if (dark) registerMonacoTheme('rose-pine', 'vs-dark', dark);
  ...
  ctx.subscriptions.push(theme.onDidChange(() => {
    if (dark) {
      ...
      cssDisposable = theme.injectCSS('rose-pine-vars', buildCSS(dark));
    }
  }));
  ...
  console.log('[Rose Pine] activated');
}
...
function deactivate() {
  ...
  console.log('[Rose Pine] deactivated');
}
