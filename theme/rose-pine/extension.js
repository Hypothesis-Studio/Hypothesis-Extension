...
function activate(ctx){const{theme,settings,commands}=ctx;const dark=l('rose-pine.json');
if(dark)ctx.subscriptions.push(theme.register(dark));if(dark)r('rose-pine','vs-dark',dark);
settings.set('workbench.colorTheme','rose-pine');
ctx.subscriptions.push(theme.onDidChange(()=>{if(dark){if(d)d.dispose();d=theme.injectCSS('rose-pine-vars',bCSS(dark))}}));
if(dark){d=theme.injectCSS('rose-pine-vars',bCSS(dark));ctx.subscriptions.push(d)}
if(commands)ctx.subscriptions.push(commands.registerCommand('rosepine.activate',()=>settings.set('workbench.colorTheme','rose-pine')));
console.log('[Rosé Pine] activated')}
function deactivate(){if(d){d.dispose();d=null}try{const w=require('electron').BrowserWindow.getAllWindows()[0];if(w&&!w.isDestroyed()){if(m.length)w.webContents.send('ext:monaco:unregister',[...m]);w.webContents.send('ext:theme:revert')}}catch{}m.length=0;console.log('[Rosé Pine] deactivated')}
