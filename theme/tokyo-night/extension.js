...
function activate(ctx) {
  ...
  ctx.subscriptions.push(theme.onDidChange((id) => {
    const def = id === 'tokyo-storm' ? storm : night;
    if (def) {
      if (cssDisposable) cssDisposable.dispose();
      cssDisposable = theme.injectCSS('tokyo-night-vars', buildCSS(def));
    }
  }));

  if (night) {
    cssDisposable = theme.injectCSS('tokyo-night-vars', buildCSS(night));
