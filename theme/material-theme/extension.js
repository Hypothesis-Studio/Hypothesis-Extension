...
function activate(ctx) {
  ...
  ctx.subscriptions.push(theme.onDidChange((id) => {
    ...
    if (def) {
      ...
      cssDisposable = theme.injectCSS('material-theme-vars', buildCSS(def));
    }
  }));

  if (dark) {
    cssDisposable = theme.injectCSS('material-theme-vars', buildCSS(dark));
    ctx.subscriptions.push(cssDisposable);
  }

  console.log('[Material Theme] activated');
}
...
function deactivate() {
  ...
  try {
    ...
    if (win && !win.isDestroyed()) {
      win.webContents.send('ext:monaco:unregister', ['material-dark', 'material-light']);
      win.webContents.send('ext:theme:revert');
    }
  } catch {}
  console.log('[Material Theme] deactivated');
}
