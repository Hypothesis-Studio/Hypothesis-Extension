...
function activate(ctx) {
  ...
  ctx.subscriptions.push(theme.onDidChange((id) => {
    const def = id === 'gruvbox-light' ? light : dark;
    if (def) {
      if (cssDisposable) cssDisposable.dispose();
      cssDisposable = theme.injectCSS('gruvbox-vars', buildCSS(def));
    }
  }));

  if (dark) {
    cssDisposable = theme.injectCSS('gruvbox-vars', buildCSS(dark));
...
function deactivate() {
  ...
  console.log('[Gruvbox] deactivated');
}
