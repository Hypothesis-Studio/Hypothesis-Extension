...
function activate(ctx) {
  ...
  ctx.subscriptions.push(theme.onDidChange(() => { if (dark) { if (cssDisposable) cssDisposable.dispose(); cssDisposable = theme.injectCSS('nord-vars', buildCSS(dark)); } }));
  if (dark) { cssDisposable = theme.injectCSS('nord-vars', buildCSS(dark)); ctx.subscriptions.push(cssDisposable); }
  if (commands) { ctx.subscriptions.push(commands.registerCommand('nord.activate', () => settings.set('workbench.colorTheme', 'nord'))); }
...
function deactivate() {
  ...
  monacoThemeIds.length = 0; console.log('[Nord] deactivated');
}
