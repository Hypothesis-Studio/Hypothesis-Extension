...
function activate(ctx) {
  ...
  const dark = loadTheme('ayu-dark.json');
  const mirage = loadTheme('ayu-mirage.json');
  const light = loadTheme('ayu-light.json');
  ...
  ctx.subscriptions.push(theme.onDidChange((id) => {
    ...
    if (def) {
      ...
      cssDisposable = theme.injectCSS('ayu-vars', buildCSS(def));
    }
  }));

  if (dark) {
    cssDisposable = theme.injectCSS('ayu-vars', buildCSS(dark));
